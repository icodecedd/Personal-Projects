...
//	int whatif; 
	...
	gotoxy(34,14);gets(stnum2); //get the input of the user
	fp=fopen("st_records.txt","r");	
	while(fscanf(fp,"%[^,],%[^,],%[^,], %f %f\n",stnum,lastn,firstn,&midterm,&finals)!=EOF)
		{
			//compares the input to the stnum in file.txt

			if(strcmp(stnum, stnum2)==0) //if the st no. is already existed
			{
		...
		//	fflush(stdin); //flush the input of the user
...
void search(){ // void function for searching a specific student
	...
	fp=fopen("st_records.txt","r"); // for reading
	system("cls");
	...
	gotoxy(34,10);gets(stnum2); //get the input of the user
	...
	while(fscanf(fp,"%[^,],%[^,],%[^,], %f %f\n",stnum,lastn,firstn,&midterm,&finals)!=EOF)
		{ 
			...
			{
				found=1;
				gotoxy(34,10);printf("%s",stnum);
				gotoxy(34,12);printf("%s",lastn);
				gotoxy(34,14);printf("%s",firstn);
				gotoxy(34,16);printf("%.2f",midterm);
				gotoxy(34,18);printf("%.2f",finals);
				double ave=(midterm+finals)/2; //compute the average
				gotoxy(34,20);printf("%.2f",ave);
				getch();
				break;
			}
		}
			...
			{
				printf(BOLD RED);
				center(25,"Student number does not exist! Press any Key..."); //if stnum is not found
				printf(RESET);
				gotoxy(65,25);getch();
			}
		fclose(fp);
...
void add_display()
{
	...
	while(fscanf(fp,"%[^,],%[^,],%[^,], %f %f\n",stnum,lastn,firstn,&midterm,&finals)!=EOF)
	{
		...
		gotoxy(9,row);printf("%d.",lctr);
		gotoxy(14,row);printf("\t  %s \t%s %s\t",stnum,firstn,lastn);
